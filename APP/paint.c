...
u8 paint_pen_color_set(u16 x, u16 y, u16 *color, u8 *caption)
{
	...
	if (twin == NULL || rbtn == NULL || okbtn == NULL || rval)
		rval = 1;
	else
	{
		// 窗口的名字和背景色
...
void paint_draw_point(u16 x, u16 y, u16 color, u8 mode)
{
	...
	if (size == 0)
		...
		gui_fill_circle(x, y, size, color);
}
...
u8 paint_play(void)
{
	...
	if (res == 0)																																															  // 确认键按下了,同时按键正常创建
	{
		...
		else // 选择一个BMP文件打开
		{
			...
			if (paintinfo.lfname == NULL)
				rval = 1; // 申请内存失败
			else
				gui_memset((u8 *)paintinfo.lfname, 0, paintinfo.lfsize);
			for (res = 0; res < 5; res++)
				lastpos[res][0] = 0XFFFF; // 坐标设定
			while (rval == 0)
			{
				tp_dev.scan(0);
				in_obj.get_key(&tp_dev, IN_TYPE_TOUCH); // 得到按键键值
		...
		while (rval == 0)
		{
			...
			if (key >= 2)	   // KEY1/WK_UP 按下,颜色设置
			{
				...
				if (key == WKUP_PRES)
					paint_pen_color_set((lcddev.width - 150) / 2, (lcddev.height - 96) / 2, &pencolor, (u8 *)pen_set_tbl[gui_phy.language][0]); // KEY1按下
				else
					paint_pen_size_set((lcddev.width - 150) / 2, (lcddev.height - 96) / 2, pencolor, &mode, (u8 *)pen_set_tbl[gui_phy.language][1]); // WK_UP按下
			...
			if (tp_dev.sta & TP_PRES_DOWN)
			{
				...
				delay_ms(1000 / OS_TICKS_PER_SEC); // 没有按键按下的时候 延时一个时钟节拍
			// 电容屏
			if (tp_dev.touchtype & 0X80) // 电容屏
			{
				for (res = 0; res < CT_MAX_TOUCH; res++)
				{
					if ((tp_dev.sta) & (1 << res))
					{
						...
						lastpos[res][0] = 0XFFFF;
				}
				if ((tp_dev.sta & 0X1F) == 0)
					delay_ms(1000 / OS_TICKS_PER_SEC); // 没有按键按下的时候 延时一个时钟节拍
			}
		}
	}
