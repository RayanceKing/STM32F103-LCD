...
REINIT:				  // 重新初始化
	...
	LED0 = 1;
	LED1 = 1;														   // 同时关闭两个LED
	if (SPI_Flash_ReadID() != W25Q64 && SPI_Flash_ReadID() != NM25Q64) // 检测不到25Q64
	{
	...
	if (res == 0) // 得到FLASH卡剩余容量和总容量
	{
		...
		system_error_show(5, ypos + fsize * (j + 1), "Flash Fat Error!", fsize);				// flash 文件系统错误
	...
	if (RTC_Init())
		system_error_show(5, ypos + fsize * (j + 1), "RTC Error!", fsize); // RTC检测
	else
	{
		RTC_Get(); // 获取时间
		LCD_ShowString(5 + okoffset, ypos + fsize * j++, lcddev.width, lcddev.height, fsize, "OK");
	...
	if (AT24CXX_Check())
		system_error_show(5, ypos + fsize * (j + 1), "24C02 Error!", fsize); // 24C02检测
	else
		LCD_ShowString(5 + okoffset, ypos + fsize * j++, lcddev.width, lcddev.height, fsize, "OK");
	// 字库检测
	...
	if (app_system_parameter_init())
		system_error_show(5, ypos + fsize * (j + 1), "Parameter Load Error!", fsize); // 参数加载
	else
		LCD_ShowString(5 + okoffset, ypos + fsize * j++, lcddev.width, lcddev.height, fsize, "OK");
